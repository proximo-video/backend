...
func NewRoom(ctx context.Context, dbClient *firestore.Client, id string, room Room) error {
	...
	if id == "" || !validID.MatchString(room.RoomId) || room.RoomId == "user" || room.RoomId == "privacy-policy" || room.RoomId == "about-us" || room.RoomId == "login" || room.RoomId == "error" || room.RoomId == "welcome" {
